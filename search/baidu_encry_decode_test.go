...
func TestGetWebconAndRealUrlFromBaiduUrl(t *testing.T) {
	re, webc, err := GetWebconAndRealUrlFromBaiduUrl("http://www.baidu.com/link?url=_NeldWbQaGXiPlAgeNSwzxsgCyn2hHh8uR-WqKqnPrXHCDsThpJCSh_-sW_nGuOzKTMlfouvFA91YjGYQQadOq")
	if err != nil {
		panic(err)
	}
	fmt.Println(re.Request.URL.String())
	fmt.Println(webc)
